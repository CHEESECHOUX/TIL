...
<br>
<details>
    <summary><strong>Q. 유저 1명이 사이트 10개를 켜놓고 동시에 로그인 시도하면 어떻게 처리될까요? <br>어떻게 탐색해야 효과적으로 찾을 수 있고 부하 없이 빠르게 찾을 수 있을까요?</strong></summary> 
    <br>
    <div style="margin-top:8px; line-height:1.6;"> <p> 같은 유저·디바이스에서 동시에 들어온 요청을 <strong>1건으로 처리</strong>합니다.<br> 리더 1개만 DB를 조회하고, 나머지 요청은 <strong>리더 결과를 재사용</strong>합니다. </p>
    <br>
    <ol>
        <li>첫 요청이 <code>login:{user}:{device}</code> 키로 <strong>짧은 락</strong>을 획득 → 유저 <strong>인덱스 조회 1회</strong> → 비밀번호 검증 → <strong>세션 upsert 1회</strong> → 결과를 <strong>잠깐 캐시</strong></li>
        <li>팔로워 처리: <strong>나머지 9개 요청</strong>은 <strong>수백 ms 대기</strong> 후, <strong>리더가 만든 결과를 그대로 응답</strong></li>
    </ol>
    <ul>
        <li><strong>풀 스캔 없이 DB 접근 = 인덱스 조회 1회 + 세션 upsert 1회</strong><br>
        같은 키로 몰린 동시 요청을 1건으로 합쳐 <strong>스탬피드 방지</strong>, DB 동시 부하 최소화</li>
        <li>전역 직렬화 없이 <strong>키별 직렬화</strong> (같은 <code>user_id+device_id</code>만 순차, 다른 키는 병렬)</li>
    </ul><br>
    <hr>
    <p><strong>* 짧은 락은 어떻게 획득할까?</strong></p>
    <pre><code>SET login:lock:{user}:{device} &lt;랜덤토큰&gt; NX PX 1500</code></pre>
    <ul>
    <li><strong>NX</strong>: 키가 없을 때만 설정(선착순 1명만 락 획득)</li>
    <li><strong>PX 1500</strong>: 1.5초 뒤 자동 만료(프로세스 장애/미해제 시 <strong>영구 잠금 방지</strong>)</li>
    <li><strong>랜덤토큰</strong>: 락 소유자 증명값 (UUID 등). 내가 잡은 락일 때만 해제/연장 가능하게 함.</li>
    </ul>
    <p>안전한 해제</p>
    <pre><code>if GET login:lock:{user}:{device} == &lt;my_token&gt; then DEL login:lock:{user}:{device}</code></pre>
    <p>→ TTL 만료 후 다른 요청이 새로 잡은 락을 <strong>실수로 지우는 사고</strong>를 방지.<br>
    필요하면 <strong>DEL 없이 TTL만</strong>으로도 가능하지만, 남은 TTL 동안 새 작업 시작이 지연될 수 있음.</p>
    <br><br>
    <p><strong>* 첫 요청이 처리된 이후, 대기 중이던 9개의 요청에 동일 응답을 어떻게 전달할까?</strong></p>
    <ul>
    <li><strong>리더</strong>: 로그인 성공 후 <code>login:result:{user}:{device}</code> 키에 <strong>토큰/세션ID</strong>를 <code>EX 5</code> 등 <strong>짧은 TTL</strong>로 저장</li>
    <li><strong>팔로워</strong>: 50~100ms 간격으로 <code>GET login:result:{user}:{device}</code> <strong>폴링</strong> → 값이 생기면 그대로 응답<br>
        타임아웃(1~2초) 시 락 재시도 또는 일반 경로로 복귀</li>
    <br>
    <li><strong>왜 짧은 TTL?</strong> 동시에 들어온 요청끼리만 결과를 공유하는 <strong>임시 버퍼</strong>이기 때문(보안/정합성/메모리 측면에서도 유리)</li>
    </ul>
    </div>
